...
start "MedVisual DIP (5000)" cmd /k "cd /d "%~dp0medvisual-dip\medvisual-dip" && run_server.bat"
...
start "MedVisual API (8000)" cmd /k "cd /d "%~dp0medvisual-api" && run_api.bat"

timeout /t 6 /nobreak >nul

rem --- 3) Web arayuzu ---
echo [3/3] Web baslatiliyor...
start "MedVisual Web (5173)" cmd /k "cd /d "%~dp0medvisual-web" && run_web.bat"
